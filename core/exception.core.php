...
class clansuite_exception extends exception
{
    ...
    public function __construct($ErrorObject, $error_head, $errorcode = 3)
    {
        // assign to parent
        parent::__construct($ErrorObject, $errorcode);

        $this->ysod($ErrorObject, $error_head, $errorcode);
     ...
     */
    public static function ysod( $ErrorObject, $error_head, $error_level = 3 )
    {
        # Header
        $errormessage    = '<html><head>';
        $errormessage   .= '<title>Yellow Screen of Death : '. $error_head .' | Errorcode: '. $error_level .'</title>';
        $errormessage   .= '<body>';
        ...
        # Fieldset Legend
        $errormessage   .= '<legend>Clansuite Exception : '. $error_head .'</legend>';
        ...
        $errormessage   .= '<tr><td><strong>Request :</strong></td><td>'.htmlentities($_SERVER['QUERY_STRING']).'</td></tr>';
