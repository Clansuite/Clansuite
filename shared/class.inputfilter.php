...
class InputFilter {
	...
	function safeSQL($source, &$connection) {
		if (is_array($source)) {
			foreach($source as $key => $value)
			if (is_string($value)) $source[$key] = $this->quoteSmart(decode($value), $connection);
			return $source;
		} else if (is_string($source)) {
			if (is_string($source)) return Inputfilter::quoteSmart(Inputfilter::decode($source), $connection);
	} else return $source;	
	}
	...
	function quoteSmart($source, &$connection) {
		...
		$source = Inputfilter::escapeString($source, $connection);
		return $source;
	}
