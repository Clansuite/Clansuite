...
{foreach key=key item=server from=$servers}
	<tr>
		<td>
			<img onclick="{ajax_update url='index.php?mod=serverlist&amp;action=get_serverdetails' update_id="server_details_`$server.server_id`" params="server_id=`$server.server_id`" callback="new Effect.SlideDown(\'serverdata_`$server.server_id`\')"}" src="{$www_root_tpl}/images/serverlist/reload_{if $server.response == true}green{else}grey{/if}.png" alt="Refresh Server {$server.server_id}" />
			<img onclick="new Effect.toggle('serverdata_{$server.server_id}', 'slide'); return false;" id="ClipDownImage_{$server.server_id}" style="display:none;" src="{$www_root_tpl}/images/serverlist/dn.gif" alt="Refresh Server {$server.server_id}" />
		</td>
		<td>{$server.server_id}</td>
		<td><img alt="Gametype Icon" src="{$www_root_tpl}/images/serverlist/gametype/{$server.gametype}.ico" /></td>
		<td>
		{if $server.image_country==''}
			<img alt="Country as Empty Icon" src="{$www_root_tpl_core}/images/empty.png" />
		{else}
		...
		<td>
			<a href="hlsw://{$server.ip}:{$server.port}"><img src="{$www_root_tpl}/images/serverlist/hlsw.ico" alt="HLSW Connect" /></a>
			{if $server.csquery_engine == 'steam'}
			<a href='steam: "-applaunch 10 -game cstrike +connect {$server.ip}:{$server.port}"'><img src="{$www_root_tpl}/images/serverlist/steam2.ico" alt="Steam Connect" /></a>
			{/if}
		</td>
	</tr>
	<tr>
