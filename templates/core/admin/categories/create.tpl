...
<form target="_self" method="POST" action="index.php?mod=admin&sub=categories&action=create">
    ...
    <table cellpadding="0" cellspacing="0" border="0" align="center" width="500">
        ...
        <tr class="tr_row1">
            ...
            <td width="1">
               <select class="input_text" name="info[icon]" onChange="document.getElementById('insert_icon').src='{$www_core_tpl_root}/images/categories/icons/'+document.getElementById('icon').options[document.getElementById('icon').options.selectedIndex].text" class="input" id="icon">
                    ...
                    {foreach key=key item=item from=$icons}
                        <option {if $smarty.post.info.icon|escape:"htmlall"==$item}selected{/if} style="background-image:url('{$www_core_tpl_root}/images/categories/icons/{$item}');background-repeat:no-repeat; padding-left:20px; height:16px; width: 135px; line-height:16px;" id="{$item}" name="{$item}">{$item}</option> );
                    {/foreach}
                </select>
            ...
            <td align="center">
                ...
                {else}
                    <img src="{$www_core_tpl_root}/images/categories/icons/{$smarty.post.info.icon|escape:"htmlall"}" id="insert_icon" border="0" width="16" height="16" class="border3d"> 
                {/if}            
            </td>
        </tr>
            ...
            <td width="1">
               <select class="input_text" name="info[image]" onChange="document.getElementById('insert_image').src='{$www_core_tpl_root}/images/categories/images/'+document.getElementById('image').options[document.getElementById('image').options.selectedIndex].text" class="input" id="image">
                    ...
                    {foreach key=key item=item from=$images}
                        <option {if $smarty.post.info.image|escape:"htmlall"==$item}selected{/if} style="background-image:url('{$www_core_tpl_root}/images/categories/images/{$item}');background-repeat:no-repeat;padding-left:55px; padding-top: 10px; height:48px; width: 100px; line-height:48px;" id="{$item}" name="{$item}">{$item}</option> );
                    {/foreach}
                </select>
            </td>
            <td align="center">
                ...
                {else}
                    <img src="{$www_core_tpl_root}/images/categories/images/{$smarty.post.info.image|escape:"htmlall"}" id="insert_image" border="0" width="48" height="48" class="border3d"> 
                {/if}
            </td>
        </tr>                
