...
<form method="post" action="index.php?mod=admin&amp;sub=categories&amp;action=edit">
    <table cellpadding="0" cellspacing="0" border="0" align="center" width="500">
        ...
        <tr class="tr_row1">
            ...
            <td colspan="2"> {* BUG CHECK THIS: name, id and vars*}
                <select name="info[module_id]" onchange="document.getElementById('insert_icon').src='{$www_core_tpl_root}/images/categories/icons/'+document.getElementById('icon').options[document.getElementById('icon').options.selectedIndex].text" class="input_text" id="icon">
                    <option value=""></option>
        ...
        <tr class="tr_row1">
            ...
            <td width="1">
               <select name="info[icon]" onchange="document.getElementById('insert_icon').src='{$www_core_tpl_root}/images/groups/icons/'+document.getElementById('icon').options[document.getElementById('icon').options.selectedIndex].text" class="input_text" id="icon">
                    <option value=""></option>
                    {foreach key=key item=item from=$icons}
                    <option {if $info.icon|escape:"html"==$item}selected="selected"{/if} style="background-image:url('{$www_core_tpl_root}/images/groups/icons/{$item}');background-repeat:no-repeat; padding-left:20px; height:16px; width: 135px; line-height:16px;" id="{$item}" value="{$item}">{$item}</option> );
                    {/foreach}
                </select>
            </td>
            <td align="center">
                ...
                {else}
                    <img src="{$www_core_tpl_root}/images/groups/icons/{$info.icon|escape:"html"}" id="insert_icon" alt="" class="border3d" />
                {/if}
            </td>
        </tr>
        <tr class="tr_row2">
            ...
            <td width="1">
               <select name="info[image]" onchange="document.getElementById('insert_image').src='{$www_core_tpl_root}/images/groups/images/'+document.getElementById('image').options[document.getElementById('image').options.selectedIndex].text" class="input_text" id="image">
                    <option value=""></option>
                    {foreach key=key item=item from=$images}
                        <option {if $info.image|escape:"html"==$item}selected="selected"{/if} style="background-image:url('{$www_core_tpl_root}/images/groups/images/{$item}');background-repeat:no-repeat;padding-left:55px; padding-top: 10px; height:48px; width: 100px; line-height:48px;" id="{$item}" value="{$item}">{$item}</option> );
                    {/foreach}
                </select>
            </td>
            <td align="center">
                ...
                {else}
                    <img src="{$www_core_tpl_root}/images/groups/images/{$info.image|escape:"html"}" id="insert_image" alt="" class="border3d" />
                {/if}
            </td>
        </tr>
        <tr class="tr_row2">
