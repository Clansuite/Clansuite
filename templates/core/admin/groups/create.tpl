...
{if $smarty.const.DEBUG eq "1"} Debugausgabe des Arrays:  {html_alt_table loop=$editgroup}   {/if}
 ...
 <form class="h3sForm" action="index.php?mod=admin&sub=groups&action=create" method="POST" target="_self">

    <fieldset>

        <input name="info[group_id]" type="hidden" value="">
	    
       <label for="group_name">
			Groupname
			<input id="group_name" name="info[name]" type="text" value="" />
		</label>
		
		<label for="group_position">
			Group Position
			<input id="group_pos" name="info[pos]" type="text" value="" />
		</label>
		
		<label for="group_description">
			Group Description
			<input id="group_description" name="info[description]" type="text" value="" />
		</label>
		...
		<label for="color">
			...
			<input name="info[color]" id="color" type="text" value="Hex-Code" />
        </label> 
