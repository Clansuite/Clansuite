    ...
    <div class="options">
        <div class="edit_button">
            ...
            {if $smarty.session.rights.cc_access == 1 && $smarty.session.rights.cc_edit_generals}
                <input class="ButtonGreen" type="button" value="{t}Edit profile (admin){/t}" onclick='{literal}Dialog.info({url: "index.php?mod=controlcenter&amp;sub=users&amp;action=cc_edit_generals", options: {method: "get"}}, {className: "alphacube", width:450, height: 400});{/literal}' />
            {/if}
        </div>
    </div>
</div>