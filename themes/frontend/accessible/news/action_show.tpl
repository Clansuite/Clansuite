	...
	<div class="news-content">
       ...
       {if isset($smarty.session.user.rights.permission_edit_news) AND isset($smarty.session.user.rights.permission_access)}
            <form action="index.php?mod=news&amp;sub=admin&amp;action=delete&amp;front=1" method="post" accept-charset="UTF-8">
                ...
                <input type="button" value="{t}Edit news{/t}" onclick='Dialog.info({url: "index.php?mod=news&amp;sub=admin&amp;action=edit&amp;id={$news.news_id}&amp;front=1", options: {method: "get"}}, {className: "alphacube", width:900, height: 600});' /> <input class="ButtonRed" type="submit" name="submit" value="{t}Delete{/t}" />
            </form>
        {/if}
	</div>
</div>
