            ...
            {foreach key=key item=server from=$servers}
                ...
                <tr>
                   <td width=50><img onclick="{ajax_update url='index.php?mod=serverlist&amp;action=get_serverdetails'
                            ...
                            src="{$www_root_theme}/images/serverlist/reload_{if $server.response == true}green{else}grey{/if}.png" 
                        ...
                        <img onclick="new Effect.toggle('serverdata_{$server.server_id}', 'slide'); return false;"
                             ...
                             style="display: none;"
                             src="{$www_root_theme}/images/serverlist/dn.gif" 
                             alt="Refresh Server {$server.server_id}" />
                   ...
                   <td>
                    ...
                    <td>
                        <a href="hlsw://{$server.ip}:{$server.port}"><img src="{$www_root_theme}/images/serverlist/hlsw.ico" class="border3d" alt="HLSW Connect" /></a>
                        {if $server.csquery_engine == 'steam'}
                        <a href='steam: "-applaunch 10 -game cstrike +connect {$server.ip}:{$server.port}"'><img src="{$www_root_theme}/images/serverlist/steam2.ico" class="border3d" alt="Steam Connect" /></a>
                        {/if}
                    </td>
                </tr>
